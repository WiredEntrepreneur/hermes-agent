...
The adapter calls `agy` with an argv array, `gemini-3.8-flash-medium`, medium effort, plan mode, sandbox, and JSON output. Its reasoning child is passed through `delegated_child_subprocess_env`: it deliberately cannot inherit Kanban mutation authority.
