...
The adapter calls `agy` with an argv array and a unique invocation-scoped project bound to the dispatcher workspace, `gemini-3.8-flash-medium`, medium effort, accept-edits mode, sandbox, and JSON output. The temporary project grants `command(*)` only inside Antigravity's terminal sandbox and is removed when the invocation exits; sandbox bypass and non-workspace file access remain review-gated. Its reasoning child is passed through `delegated_child_subprocess_env`: it deliberately cannot inherit Kanban mutation authority.
